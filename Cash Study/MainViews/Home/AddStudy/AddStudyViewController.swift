...
class AddStudyViewController: UIViewController, UITextFieldDelegate {
    ...
    var isAlreadyExist : Bool = false {
        didSet {
            let vc = ObAlertViewController(mainMsg: "해당 공부는 이미 존재합니다\n제목을 수정해주세요", subMsg: "", heightValue: 0.2, btnTitle: "확인", isTimer: false, isMypage: false)
    ...
    var editRpSuccess : Bool = false {
        didSet {
            let vc = ObAlertViewController(mainMsg: "오늘 일자 이후의\n공부 반복하기를 변경합니다", subMsg: "", heightValue: 0.2, btnTitle: "확인", isTimer: false, isMypage: false)
            vc.modalPresentationStyle = .overFullScreen
            present(vc, animated: true)
        }
    }
    ...
    @IBAction func addStudyBtnTapped(_ sender: UIButton) {
        ...
        if titleTF.text?.isEmpty == true {
            ...
            if repeatSd.value == 1 {
                if sDate > eDate {
                    let vc = ObAlertViewController(mainMsg: "종료일은 시작일보다\n이전일 수 없습니다", subMsg: "", heightValue: 0.2, btnTitle: "확인", isTimer: false, isMypage: false)
                    vc.modalPresentationStyle = .overFullScreen
                    present(vc, animated: true)
                } else {
                    if isEdit {
                        ...
                        if dt >= sDate {
                            let vc = ObAlertViewController(mainMsg: "오늘 이후의 날짜만 선택 가능합니다", subMsg: "", heightValue: 0.2, btnTitle: "확인", isTimer: false, isMypage: false)
                            vc.modalPresentationStyle = .overFullScreen
                            present(vc, animated: true)
                        } else {
